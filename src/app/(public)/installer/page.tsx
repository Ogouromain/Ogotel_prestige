...
const setupSchema = z.object({
  ...
  setupSecret: z.string().min(1, "Le code de sécurité est requis."),
...
export default function InstallerPage() {
  ...
  async function onSubmit(data: SetupValues) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) {
          toast.error(result.error || "Accès refusé. Vérifiez votre code de sécurité.");
        } else {
              ...
              <div className="space-y-3">
                <div className="rounded-lg bg-ivory p-4 text-sm space-y-2">
                  ...
                  <p className="text-xs text-slate mt-2">
                    Cela créera toutes les tables, enums, triggers et fonctions RPC.
                  </p>
                </div>
              </div>
            </SetupStep>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input
                              type="password"
                              placeholder="Entrez le SETUP_SECRET configuré"
                              {...field}
                          ...
                          <p className="text-xs text-slate mt-1">
                            Ce code est défini dans la variable d&apos;environnement SETUP_SECRET.
                          </p>
                        </FormItem>
                      )}
                    />
