...
function ConnexionPageInner() {
  ...
  // ─── Check setup status + init Supabase ─────────────────────────
  useEffect(() => {
    async function checkSetup() {
      // Pre-initialize Supabase client with runtime config
      try {
        await initSupabaseClient();
      } catch {
        // Supabase not configured — the setup check will detect it
      }
  ...
  async function onSubmit(data: LoginValues) {
    ...
    try {
      ...
      if (msg.includes("Supabase n'est pas configuré")) {
        toast.error("Supabase n'est pas configuré sur ce déploiement. Contactez l'administrateur.");
      } else {
