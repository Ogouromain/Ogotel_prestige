...
/**
 * Valide le secret de setup (obligatoire en production si SETUP_SECRET est défini).
 */
function validateSetupSecret(request: NextRequest): { valid: boolean; error?: string } {
  const configuredSecret = process.env.SETUP_SECRET;

  // En production, SETUP_SECRET doit être défini
  if (process.env.NODE_ENV === "production" && !configuredSecret) {
    return {
      valid: false,
      error:
        "SETUP_SECRET n'est pas configuré. Définissez cette variable dans les variables d'environnement.",
    };
  }

  // Si SETUP_SECRET est configuré, il doit être fourni et correspondre
  if (configuredSecret) {
    const provided = request.headers.get("x-setup-secret");
    if (!provided) {
      return { valid: false, error: "Secret de setup manquant." };
    }
    if (provided !== configuredSecret) {
      return { valid: false, error: "Secret de setup invalide." };
    }
  }
...
export async function POST(request: NextRequest) {
  try {
    // ─── 0. Valider le secret de setup ─────────────────────────
    const secretCheck = validateSetupSecret(request);
    if (!secretCheck.valid) {
      return NextResponse.json(
        { error: secretCheck.error, step: "forbidden" },
        { status: 403 }
      );
    }
    ...
    // Audit log
    console.warn(
      `[SECURITY] Super admin créé — email: ${body.email}, ip: ${request.headers.get("x-forwarded-for") || "unknown"}`
    );
 ...
 *
 * 🔒 Ne révèle PAS l'email de l'admin (information sensible).
 */
